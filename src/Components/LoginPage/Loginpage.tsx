...
import {
  ...
  AuthResponse,
  ValidationErrors,
} from "../../Models/Loginpage.model";

export default function LoginPage({ setAuthorized }: LoginPageProps) {
  ...
  function validateForm(): boolean {
    let valid = true;
    const newErrors: ValidationErrors = { username: "", password: "" };
  ...
  async function handleLogin(e: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      if (!baseURL) {
        console.error("VITE_ECR_BASE_URL is not defined in the environment.");
        throw new Error(
          "API base URL is missing. Please set VITE_ECR_BASE_URL."
        );
      }

      const response: AxiosResponse<AuthResponse> = await axios.post(
        `${baseURL}/api/auth/generateAuthToken`,
        formData.toString(),
        {
  ...
  return (
    <div className="m-auto w-50">
      ...
      <form data-testid="login-form" onSubmit={handleLogin} noValidate>
        {loginError && (
